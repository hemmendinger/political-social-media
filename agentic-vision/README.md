...
- `knowledge-seed/` — the knowledge layer already populated from what the repository knows today: three
  source dossiers with 26 addressable quirks, a 94-item structured backlog (JSON, because the pipeline's data handling is standard-library only and `ts status` must read it) (migrating `TODO.md`, the dead-code review, and the
...
- The removed search asks trumpstruth for posts *created* in the last 14 days (the site filters by creation
  ...
  at removal. The fix costs 0 to 4 extra search-page requests per run, plus one status page per removal it
  newly finds. (`05` section 3, B-029, D-017)
...
- The 1,172 "ambiguous handle" rows that head `TODO.md` are 1,157 self-reposts the parser already resolves
  by exact prefix plus 15 glued other handles (8 of which appear as handles in API- or trumpstruth-sourced
  fields elsewhere in the data); the P0 item was a 15-row problem misdirected by its own check definition.
  (B-054, B-001, L-008)
- Knowledge is filed by document type, not by subject: the same CNN quirk appears in at least seven files, no
  lesson names its test, decisions are one-liners in OPERATIONS with their rationale outside the repository,
  and the one audit was never acted on. (`07`, B-060 to B-064)
...
- A source's name is a string literal on about 37 lines across eight modules; an unregistered host is paced
  ...
  path. (`05` sections 1.1 to 1.3, B-043 to B-045)
...
- `max_trumpstruth_id` is assigned only after the walk, although the docstring says per id and SPEC says per
  page; a crash mid-walk re-fetches. (B-015)
