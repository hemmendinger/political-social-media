...
The bot's commit step stays `git add data output STATUS.md`. A coherence test in `ts verify` fails if a
...
- Never delete a ledger. `redo-history` moves; it does not delete.
