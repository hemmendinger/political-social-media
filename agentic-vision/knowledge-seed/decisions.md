...
| D-011 | Schemas live at the repo root `schemas/` | proposed | 2026-09-12 | check_data and build_db must import them; docs generate from them | accept |
...
| D-013 | Drop pandas from `requirements.txt` | proposed | 2026-09-12 | imported by nothing; installed on every cloud run | move to `requirements-dev.txt` |
...
| D-017 | Removed-search window policy | proposed | 2026-09-12 | the search filters by post creation date; the 14-day window misses removals of older posts (35 of 98 known deletions were older than 14 days at removal) | search since 2022-01-01 on every run while total removals are under about 500 (one to five pages), then 14 days daily plus a weekly full sweep; record the sweep window on the run record |
