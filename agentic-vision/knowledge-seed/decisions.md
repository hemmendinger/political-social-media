...
| D-008 | Role of the api collector given the cloud 403 | proposed | 2026-09-11 | it cannot run from GitHub; the API is the only source that can verify historical existence, so B-003 needs it (and a full-walk mode the collector lacks today) | keep, desktop-only by profile; cloud never probes; monthly canary records whether the block persists |
| D-009 | Wire or remove the trumpstruth total check | proposed | 2026-09-12 | `trumpstruth_totals` has no caller; README and SPEC present the check as live, OPERATIONS hedges with "only when passed in" | wire it: one stats-page request per run; it is the only completeness signal independent of the API |
| D-010 | Stop committing `output/posts.csv` | proposed | 2026-09-12 | 12.7 MB rewritten per run and committed whenever posts changed; pure derivation of `data/` | untrack; `ts build --csv` on demand; weekly release asset if a URL is wanted |
...
| D-015 | One dispatcher (`scripts/ts.py`) as the control surface | proposed | 2026-09-12 | six entry points, no dry-run, no cost line, structured output only from check_data | accept; the old entry points stay until phase 2 |
