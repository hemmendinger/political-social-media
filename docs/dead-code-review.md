...
|---|---|---|
| 1 | Unused imports | 5 |
| 2 | Functions/classes/constants defined but never referenced anywhere | 0 |
...
| 9 | Write-only `state.json` keys / unread record fields | 9 + 7 = 16 |
...
**None found.** Every top-level `def`/`class`/constant in `scripts/*.py` is reached from at least
...
| `sources.api.last_ok_at` | `scripts/collect_api.py:54,220` | High | Same |
...
| `sources.api.reachable` | `scripts/collect_api.py:54,96,108` | High | Remove, or keep as an operator-visible health flag |
| `sources.api.last_probe_status` | `scripts/collect_api.py:54,97,109` | High | Same |
| `sources.api.last_probe_at` | `scripts/collect_api.py:55,98,110` | High | Same |

All of the above are asserted on in `tests/test_collect_api.py`/`tests/test_store.py` (the tests
...
| `pinned` | `scripts/parsers.py:698` | Medium | Verify — only touched by `check_data.py:110-111`'s boolean-type check, not by any metric |
